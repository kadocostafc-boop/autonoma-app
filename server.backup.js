...
app.get('/api/profissional/:id/avaliacoes', (req, res) => {
  const id = String([req.params.id](http://req.params.id/));
  const db = loadDB();
  const prof = db.find(p => String([p.id](http://p.id/)) === id);
...
[app.post](http://app.post/)('/api/profissional/:id/avaliar', express.json(), (req, res) => {
  const id = String([req.params.id](http://req.params.id/));
...
async function sendWhatsAppTemplate(to) {
  const url = `[https://graph.facebook.com/v22.0/${process.env.WA_PHONE_ID}/messages`]([https://graph.facebook.com/v22.0/$%7Bprocess.env.WA_PHONE_ID%7D/messages](https://graph.facebook.com/v22.0/$%7Bprocess.env.WA_PHONE_ID%7D/messages));
  const body = {
...
app.get("/wa/template", async (req, res) => {
  try {
    const to = ([req.query.to](http://req.query.to/) || "5521971891276").replace(/\D/g, "");
    const url = `[https://graph.facebook.com/v22.0/${process.env.WA_PHONE_ID}/messages`]([https://graph.facebook.com/v22.0/$%7Bprocess.env.WA_PHONE_ID%7D/messages](https://graph.facebook.com/v22.0/$%7Bprocess.env.WA_PHONE_ID%7D/messages));
    const payload = {
      messaging_product: "whatsapp",
      to,
...
app.get("/wa/pin", async (req, res) => {
  try {
    const to = String(([req.query.to](http://req.query.to/) || "")).replace(/\D/g, "");
...
app.get("/wa/template", async (req, res) => {
  try {
    ...
    const to = String([req.query.to](http://req.query.to/) || "5521971891276").replace(/\D/g, "");
    const url = `[https://graph.facebook.com/v22.0/${process.env.WA_PHONE_ID}/messages`]([https://graph.facebook.com/v22.0/$%7Bprocess.env.WA_PHONE_ID%7D/messages](https://graph.facebook.com/v22.0/$%7Bprocess.env.WA_PHONE_ID%7D/messages));
...
// =========================[ HTML helpers ]=====================
const htmlMsg = (title, text, backHref="/") =>`<!doctype html><meta charset="utf-8"><link rel="stylesheet" href="/css/app.css"><div class="wrap"><div class="card"><h1>${escapeHTML(title)}</h1><p class="meta">${escapeHTML(text||"")}</p><a class="btn" href="${escapeHTML(backHref)}">Voltar</a></div></div>`;

const htmlErrors = (title, list, backHref="/") =>`<!doctype html><meta charset="utf-8"><link rel="stylesheet" href="/css/app.css"><div class="wrap"><div class="card"><h1>${escapeHTML(title)}</h1><ul>${(list||[]).map(e=>`
- ${escapeHTML(e)}
`).join("")}</ul><a class="btn" href="${escapeHTML(backHref)}">Voltar</a></div></div>`;
...
app.get(["/perfil.html","/perfil"], (req,res)=>{
  const id = Number([req.query.id](http://req.query.id/)||"");
  if (id) return res.redirect(301, `/profissional/${id}`);
  return res.redirect(302, "/clientes.html");
});
app.get("/clientes", (*req,res)=> res.redirect(301, "/clientes.html"));
app.get("/cadastro", (*req,res)=> res.redirect(301, "/cadastro.html"));
...
(function fixDB(){
  const db = readDB();
  let changed=false;
  let nextId = db.reduce((m,p)=>Math.max(m, Number([p.id](http://p.id/)||0)),0)+1;
  for (const p of db){
    if(p.id){ [p.id](http://p.id/)=nextId++; changed=true; }
...
const storage = multer.diskStorage({
  ...
  filename:    (*, file, cb) => cb(null, `${Date.now()}-${Math.round(Math.random()*1e9)}${path.extname(file.originalname)}`)
});
const upload = multer({
  storage,
  limits:{ fileSize: 3*1024*1024 },
  fileFilter: (*, file, cb) => file.mimetype?.startsWith("image/") ? cb(null,true) : cb(new Error("Apenas imagens (JPG/PNG)."))
});
...
[app.post](http://app.post/)("/cadastrar",
  ...
  (req,res)=>{
    try{
      ...
      req.session.lastCreatedProId = [novo.id](http://novo.id/);
      res.redirect(`/profissional/${[novo.id](http://novo.id/)}`);
    } catch(err){
...
app.get("/api/profissionais", (req, res) => {
  try{
    ...
    const page  = Math.max(1, Number([req.query.page](http://req.query.page/)||1));
...
app.get("/api/profissionais/:id", (req, res) => {
  const id = Number([req.params.id](http://req.params.id/) || "0");
  if (!Number.isFinite(id) || id <= 0) return res.status(400).json({ ok:false, error:"id inválido" });
  const db = readDB();
  const p = db.find(x => Number([x.id](http://x.id/)) === id && !x.excluido);
...
[app.post](http://app.post/)("/api/avaliacoes", reviewsLimiter, (req,res)=>{
  try{
    ...
    const db = readDB();
    const p = db.find(x=> Number([x.id](http://x.id/))===proId && !x.excluido);
    ...
    const recent = (p.avaliacoes||[]).some(a=>{
      const t = Date.parse([a.at](http://a.at/)||"");
...
[app.post](http://app.post/)("/profissional/:id/avaliar", (req,res)=>{
  const id = Number([req.params.id](http://req.params.id/) || "0");
  try{
...
app.get("/avaliar/:id", (req,res)=>{
  ...
  const itens = avals.map(a=>{
    ...
    const when = [a.at](http://a.at/) ? new Date([a.at](http://a.at/)).toLocaleString() : "";
    return `
      <li class="cmt">
        <div class="cmt-head">
    ...
    <div class="card">
      ...
      <div class="grid">
        <div class="col">
          ...
          <form method="POST" action="/profissional/${[p.id](http://p.id/)}/avaliar" novalidate>
            ...
            <div class="row">
              ...
              <a class="btn ghost" href="/perfil.html?id=${[p.id](http://p.id/)}">Voltar ao perfil</a>
            </div>
...
app.get("/profissional/:id", (req,res)=>{
  const idNum = Number([req.params.id](http://req.params.id/) || "0");
...
[app.post](http://app.post/)("/api/track/visit/:id", (req,res)=>{
  const id = Number([req.params.id](http://req.params.id/)||"0");
  const db = readDB();
  const p = db.find(x=> Number([x.id](http://x.id/))===id && !x.excluido);
  if (p){
...
[app.post](http://app.post/)("/api/track/call/:id", (req,res)=>{
  const id = Number([req.params.id](http://req.params.id/)||"0");
  const db = readDB();
  const p = db.find(x=> Number([x.id](http://x.id/))===id && !x.excluido);
  if (p){
...
[app.post](http://app.post/)("/api/track/qr/:id", (req,res)=>{
  const id = Number([req.params.id](http://req.params.id/)||"0");
  const db = readDB();
  const p = db.find(x=> Number([x.id](http://x.id/))===id && !x.excluido);
  if (p){
...
function scoreTop10(p){
  ...
  const calls = (p.callsLog||[]).filter(x=> weekKeyFor([x.at](http://x.at/))===thisWeek ).length;
  const sevenAgo = Date.now() - 6*86400000;
  const visits = (p.visitsLog||[]).filter(x=> Date.parse([x.at](http://x.at/))>=sevenAgo ).length;
...
app.get("/api/top10", (req,res)=>{
  ...
  list = list.map(p => ({ ...p, topScore: scoreTop10(p) }))
             ...
             .slice(0,10)
             .map(p => ({
               id: [p.id](http://p.id/), nome:p.nome, foto:p.foto||"",
               servico:p.servico||p.profissao||"",
...
[app.post](http://app.post/)("/api/painel/reset-pin/request", async (req,res)=>{
  try{
...
[app.post](http://app.post/)("/api/painel/reset-pin/confirm", (req,res)=>{
  try{
    const phoneRaw = String(req.body?.phone||"").trim();
    ...
    req.session.painel = { ok:true, proId: [p.id](http://p.id/), when: Date.now() };
...
app.get("/api/painel/me", (req, res) => {
  try {
    ...
    if (req.session?.painel?.ok) {
      pro = db.find(p => Number([p.id](http://p.id/)) === Number(req.session.painel.proId) && !p.excluido);
    }
    if (!pro) {
      ...
      if (auth.startsWith("Bearer ")) {
        ...
        if (tok && /^\d{12,13}$/.test(tok)) {
          ...
          if (pro) req.session.painel = { ok:true, proId: [pro.id](http://pro.id/), when: Date.now() };
        }
      }
    }
...
[app.post](http://app.post/)("/api/painel/login", loginLimiter, (req, res) => {
  try {
    ...
    req.session.painel = { ok:true, proId: [pro.id](http://pro.id/), when: Date.now() };
...
[app.post](http://app.post/)("/api/painel/set-pin", (req, res) => {
  ...
  const db = readDB();
  const p = db.find(x => Number([x.id](http://x.id/)) === Number(s.proId));
  if (!p) return res.status(404).json({ ok:false });
...
[app.post](http://app.post/)("/api/painel/logout", (req,res)=>{ if (req.session) req.session.painel=null; res.json({ ok:true }); });
...
app.get("/api/painel/state", (req,res)=>{
  ...
  res.json({
    ok:true,
    pro:{
      id:[p.id](http://p.id/), nome:p.nome, plano:p.plano,
...
[app.post](http://app.post/)("/api/painel/radar", (req,res)=>{
  const s = req.session?.painel;
  if (!s?.ok || !s?.proId) return res.status(401).json({ ok:false });

  const { on, durationHours } = req.body || {};
  const db = readDB();
  const p = db.find(x=> Number([x.id](http://x.id/))===s.proId);
  if (!p) return res.status(404).json({ ok:false });
...
[app.post](http://app.post/)("/api/painel/radar/autooff", (req,res)=>{
  const s = req.session?.painel; if (!s?.ok) return res.status(401).json({ ok:false });
  const db = readDB(); const p = db.find(x=> Number([x.id](http://x.id/))===s.proId); if (!p) return res.status(404).json({ ok:false });
...
[app.post](http://app.post/)("/api/painel/raio", (req,res)=>{
  const s = req.session?.painel; if (!s?.ok) return res.status(401).json({ ok:false });
  const db = readDB(); const p = db.find(x=> Number([x.id](http://x.id/))===s.proId); if (!p) return res.status(404).json({ ok:false });
  const lim = proLimits(p);
...
[app.post](http://app.post/)("/api/painel/cidades", (req,res)=>{
  const s = req.session?.painel; if (!s?.ok) return res.status(401).json({ ok:false });
  const db = readDB(); const p = db.find(x=> Number([x.id](http://x.id/))===s.proId); if (!p) return res.status(404).json({ ok:false });
  const lim = proLimits(p);
...
[app.post](http://app.post/)("/api/painel/payment-prefs", (req,res)=>{
  const s = req.session?.painel; if (!s?.ok) return res.status(401).json({ ok:false });
  const db = readDB(); const p = db.find(x=> Number([x.id](http://x.id/))===s.proId); if (!p) return res.status(404).json({ ok:false });
...
[app.post](http://app.post/)("/api/painel/update",
...
app.get("/api/painel/export.csv", (req,res)=>{
  ...
  const lines = [
    ["id",[p.id](http://p.id/)],["nome",p.nome],["whatsapp",p.whatsapp],["cidade",p.cidade],["bairro",p.bairro],
...
[app.post](http://app.post/)("/api/checkout/intent", (req,res)=>{
  try{
...
[app.post](http://app.post/)("/api/checkout/pay/:pid", (req,res)=>{
...
app.get("/api/checkout/pro/:id", (req,res)=>{
  ...
  res.json({
    ok:true,
    pro:{ id:[p.id](http://p.id/), nome:p.nome, receiveViaApp:!!p.receiveViaApp },
    options: { pix:PIX_ENABLED, card:CARD_ENABLED, fees:{ cardPercent:FEE_CARD_PERCENT, pixPercent:FEE_PIX_PERCENT } }
  });
});

// ===========================[ Denúncias ]======================
[app.post](http://app.post/)("/api/denuncias", (req,res)=>{
  try{
    const body = req.body||{};
...
app.get("/api/favoritos", (req,res)=>{
  try{
    ...
    const db = readDB();
    const items = ids
      .map(id => db.find(p => Number([p.id](http://p.id/))===Number(id) && !p.excluido))
      .filter(Boolean)
      .map(p=>{
        const notas = (p.avaliacoes||[]).map(a=>Number(a.nota)).filter(n=>n>=1&&n<=5);
        const rating = notas.length ? (notas.reduce((a,b)=>a+b,0)/notas.length) : 0;
...
[app.post](http://app.post/)("/api/favoritos/toggle", (req,res)=>{
  try{
    const uid = ensureFavUID(req,res);
    const id = Number((req.body && [req.body.id](http://req.body.id/)) || (req.query && [req.query.id](http://req.query.id/)) || "0");
    if (!Number.isFinite(id) || id<=0) return res.status(400).json({ ok:false, error:"id inválido" });

    const db = readDB();
    const exists = db.some(p => Number([p.id](http://p.id/))===id && !p.excluido);
...
// POST /admin/login
[app.post](http://app.post/)("/admin/login", loginLimiter, (req, res) => {
...
// Logout
[app.post](http://app.post/)("/admin/logout", (req,res)=>{ if (req.session) req.session.isAdmin = false; res.redirect("/admin/login"); });
...
function adminBuildList(query){
  ...
  items.sort((a,b)=> Number([b.id](http://b.id/))-Number([a.id](http://a.id/)));
  return items.map(p=>({
    id:[p.id](http://p.id/), nome:p.nome, servico:p.servico||p.profissao||"",
    cidade:p.cidade||"", bairro:p.bairro||"",
...
app.get("/api/admin/profissionais", requireAdmin, (req,res)=>{
  try{
    ...
    items.sort((a,b)=>{
      let v=0;
      switch (sort){
        case "recent":     v = Number([b.id](http://b.id/))-Number([a.id](http://a.id/)); break;
        ...
        default:           v = Number([b.id](http://b.id/))-Number([a.id](http://a.id/));
      }
      return dirAsc ? v : -v;
    });
...
[app.post](http://app.post/)("/api/admin/profissionais/:id/suspender", requireAdmin, (req,res)=>{
  try{
    const id = Number([req.params.id](http://req.params.id/)||"0");
    const db = readDB();
    const p = db.find(x=> Number([x.id](http://x.id/))===id);
    if (!p) return res.status(404).json({ ok:false });
    p.suspenso = true;
...
[app.post](http://app.post/)("/api/admin/profissionais/:id/ativar", requireAdmin, (req,res)=>{
  try{
    const id = Number([req.params.id](http://req.params.id/)||"0");
    const db = readDB();
    const p = db.find(x=> Number([x.id](http://x.id/))===id);
...
[app.post](http://app.post/)("/api/admin/profissionais/:id/restaurar", requireAdmin, (req,res)=>{
  try{
    const id = Number([req.params.id](http://req.params.id/)||"0");
    const db = readDB();
    const p = db.find(x=> Number([x.id](http://x.id/))===id);
...
app.get("/api/admin/denuncias", requireAdmin, (req,res)=>{
  try{
    ...
    arr.forEach(d=>{
      ...
      if (!d.createdAt) d.createdAt = [d.at](http://d.at/) || nowISO();
    });
...
// === Rotas de avaliações (compat adicionais) ===
app.get('/api/profissional/:id/avaliacoes', (req, res) => {
